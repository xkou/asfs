...
if(empty($dopost))
{
	...
	include(DEDEMEMBER."/templets/newinfo2.htm");
	exit();
}
/*------------------------------
...
else if($dopost=='save')
{
	...
	if(defined('UC_APPID'))
	{
		...
		$row = $dsql->GetOne("select uid from uchome_space where username='".$cfg_ml->M_UserName."'");
		$spaceid = "";
		if( $row ) $spaceid= $row['uid'];

		$feed['title_template'] = '<b>{username}</b> 发布信息 {title}';
		$feed['title_data'] = array('username' => '<a href="/u/space.php?uid='.$spaceid.'" >'.$cfg_ml->M_UserName.'</a>', 'title' => "<a href=\"$artUrl\">$title</a>" );
		$feed['body_template'] = '<b>{subject}</b><br>{message}';		
		$feed['body_data'] = array('subject' => "<a href=\"$artUrl\">$title</a>", 'message' => cn_substr(strip_tags(preg_replace("/\[.+?\]/is", '', $description)), 150));		
	...
	//返回成功信息
	$msg = "
		<h2>发布的信息会在审核后一小时内显示出来,谢谢合作.</h2><p />
	　　请选择你的后续操作：
		<a href='/'><u>返回网站首页</u></a>
		&nbsp;&nbsp;
		<a href='newinfo.php'><u>继续发布内容</u></a>
		&nbsp;&nbsp;
	...
	{
		$msg.="
		<a href='/member/index_do.php?fmdo=user&dopost=regnew'><u>注册会员以便于管理您发布的内容</u></a>
		";
